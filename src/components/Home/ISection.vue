<template lang="html">
  <div class="container" @click="more(movieObject.basicId)">
  <!-- <div class="container" @click=""> -->
      <img :src="BURL+movieObject.basicPic" :alt="movieObject.basicTitle">
      <div class="text-container">
        <div class="panel">
          <div class="panel-img">
            <img :src="BURL+movieObject.basicPic" :alt="movieObject.basicTitle">
          </div>
          <div class="panel-text">
            <p class="title-cn">{{movieObject.articleName}}</p>
            <p class="title-en">{{movieObject.articleEngName}}</p>
            <p class="director">导演：{{movieObject.articleSource}}</p>
            <p class="stars">

              <img class="imdbmark" src="static/img/IMDb@2x.png">
            </p>
            <p class="tips">
              <span>{{movieObject.movieYear||"2017"}}</span>
              <span>{{movieObject.movieCountry||"国家未知"}}</span>
              <span>{{movieObject.movieType||"类型未知"}}</span>
            </p>
          </div>
        </div>
      </div>
  </div>
</template>

<script>
export default {
  name: 'ISection',
  props: [
    'movieObject'
  ],
  data() {
    return {
      msg: 'this is ISection!',
      BURL: BURL
    }
  },
  methods: {
    more(id) {
      this.$router.push("/detail/" + id)
    }
  }
}
</script>

<style lang="less">
.container {
    height: 100%;
    position: relative;
    img {
        z-index: -1;
        float: left;
        width: 100%;
        height: 100%;
    }
    .text-container {
        position: absolute;
        left: 0;
        bottom: 0;
        width: 100%;
        height: 10rem;
        text-align: center;
        .panel {
            position: relative;
            height: 8rem;
            width: 93%;
            margin: 0 auto;
            border-radius: 10px;
            background-color: #fff;
            box-shadow: 5px 5px 10px #ddd;
            .panel-img {
                position: absolute;
                bottom: 1rem;
                left: 1rem;
                width: 6rem;
                height: 8rem;
                img {
                    border-radius: 8px;
                }
            }
            .panel-text {
                margin-left: 37%;
                text-align: left;
                font-size: 0.75rem;
                .title-cn {
                    padding-top: 0.5rem;
                    font-size: 1.625rem;
                }
                .director,
                .stars,
                .tips,
                .title-en {
                    line-height: 1.25rem;
                }
                .stars {
                    line-height: 1rem;
                    height: 1rem;
                    color: #FA4455;
                    .imdbscore {
                        float: left;
                    }
                    img {
                        height: 1rem;
                        width: 2rem;
                        float: left;
                    }
                    .imdbstars {
                        position: relative;
                        width: 6rem;
                        height: 0.94rem;
                        float: left;
                        display: block;
                        .imdbstars1 {
                            display: block;
                            position: absolute;
                            left: 0;
                            border-top: 20%;
                            border-color: #fff;
                            height: 80%;
                            width: 100%;
                            background-image: url("data:image/png;base64,");
                            background-size: 80%;
                            background-position: left;
                            background-repeat: no-repeat;
                            z-index: 2;
                        }
                        .starfill1 {
                            float: left;
                            height: 80%;
                            background-color: red;
                            z-index: 1;
                        }
                        .starfill2 {
                            float: left;
                            width: 6rem;
                            height: 80%;
                            background-color: #aaa;
                            z-index: 1;
                        }
                        .starfill1.w0 {
                            width: 7%;
                        }
                        .starfill2.w0 {
                            width: 72%;
                        }
                        .w1 {
                            width: 7%;
                        }
                        .w0 {
                            width: 0;
                        }
                        .w2 {
                            width: 14%;
                        }
                    }

                }
                .tips {
                    span {
                        display: inline-block;
                        border: 1px solid #989A9C;
                        border-radius: 2px;
                        padding: 1px;
                        color: #989A9C;
                        font-size: 0.5rem;
                        padding: 0 0.1rem;
                    }
                }

            }
        }
    }

}
</style>
